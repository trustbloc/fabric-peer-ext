...
func (c *Client) Endorse(req *api.Request) (*channel.Response, error) {
	resp, err := c.channelClient.Query(
		channel.Request{
...
func (c *Client) EndorseAndCommit(req *api.Request) (*channel.Response, error) {
	resp, err := c.channelClient.Execute(
		channel.Request{
			ChaincodeID:  req.ChaincodeID,
			Fcn:          string(req.Args[0]),
			Args:         req.Args[1:],

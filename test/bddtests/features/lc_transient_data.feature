...
Feature:
  ...
  Scenario: Put and get transient data
    ...
    Given off-ledger collection config "coll1_upgrade" is defined for collection "collection1" as policy="OR('Org1MSP.member','Org2MSP.member')", requiredPeerCount=1, maxPeerCount=2, and timeToLive=1m
    ...
    And collection config "coll3" is defined for collection "collection3" as policy="OR('Org1MSP.member','Org2MSP.member')", requiredPeerCount=1, maxPeerCount=2, and blocksToLive=1000
    Then chaincode "tdata_examplecc", version "v1.0.3", package ID "tdata_examplecc:v1", sequence 3 is approved and committed by orgs "peerorg1,peerorg2" on the "mychannel" channel with endorsement policy "AND('Org1MSP.member','Org2MSP.member')" and collection policy "tdata_coll1_upgrade,tdata_coll2_upgrade,coll3"
    Given we wait 10 seconds
    ...
    Then chaincode "tdata_examplecc_2", version "v1", package ID "tdata_examplecc_2:v1", sequence 1 is approved and committed by orgs "peerorg1,peerorg2" on the "mychannel" channel with endorsement policy "AND('Org1MSP.member','Org2MSP.member')" and collection policy "tdata_coll2"
    Given we wait 10 seconds
    ...
    Then chaincode "tdata_examplecc_2", version "v1", package ID "tdata_examplecc_2:v1", sequence 1 is approved and committed by orgs "peerorg1,peerorg2" on the "yourchannel" channel with endorsement policy "AND('Org1MSP.member','Org2MSP.member')" and collection policy "tdata_coll2"
    Given we wait 10 seconds

...
Feature: Lifecycle txn
  ...
  Scenario: Endorsements using TXN service
    ...
    Given collection config "privColl" is defined for collection "collection3" as policy="OR('Org1MSP.member','Org2MSP.member')", requiredPeerCount=1, maxPeerCount=2, and blocksToLive=3
    Then chaincode "target_cc" is installed from path "fixtures/testdata/chaincode/e2e_cc" to all peers
    And the response is saved to variable "targetCCPackageID"

    And chaincode "target_cc", version "v1", package ID "${targetCCPackageID}", sequence 1 is approved and committed by orgs "peerorg1,peerorg2" on the "mychannel" channel with endorsement policy "AND('Org1MSP.member','Org2MSP.member')" and collection policy "privColl"
